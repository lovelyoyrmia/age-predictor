...
pages = st.sidebar.selectbox("Select Page", ["Age Predictor", "About"])

if pages == "Age Predictor":
    image_upload = st.file_uploader("Pick Your Image", type=["jpg", "jpeg", "png"])
    if image_upload is not None:
        if load_imgpath(image_upload):
            ...
            st.error("Image Path Error")
    else:
        st.warning("Select your photo to predict how old you are")
else:
    st.text("About Me")
    st.markdown(
        """
